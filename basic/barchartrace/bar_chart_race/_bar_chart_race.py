...
def get_image_code_value():
    # Full Name -> Short Name 
    codeValuedf = load_dataset('code_value', index='code')
    # print(codeValuedf)
    return codeValuedf

def get_image_code(root_folder):
    titledf = load_dataset(root_folder, index='code')
    # print(titledf)
    return titledf
...
def get_image_name(code, col_name):
    ...
    ----------
        str
    '''
    #print(code.loc[col_name.lower()].loc['value'])
    col_name = col_name.lower().replace('_', ' ')

    split_name = col_name.split('.')
    if len(split_name) > 1:
        # img_name = split_name
        img_name = code.loc[split_name].loc['value']
    else:
        # img_name = col_name + '.png'
        img_name = code.loc[col_name].loc['value'] + '.png'
...
class _BarChartRace(CommonChart):
    ...
    def __init__(self, df, filename, orientation, sort, n_bars, fixed_order, fixed_max,
        ...
        self.code_value = get_image_code_value()
        self.code = get_image_code(img_label_folder)
    ...
    def add_bar_labels(self, ax, bar_location, bar_length):
        if self.bar_textposition:
            ...
            delta = .05 if self.bar_textposition == 'outside' else -.01
