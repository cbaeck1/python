# 정형 데이터 다루기
# 정형 데이터(structured data)를 다루는 방법을 소개합니다(예를 들어 CSV에서 읽은 표 형식의 데이터)
# 케라스를 사용하여 모델을 정의하고 특성 열(feature column)을 사용하여 CSV의 열을 모델 훈련에 필요한 특성으로 매핑하겠습니다. 
# 이 튜토리얼은 다음 내용을 포함합니다:
#  1) 판다스(Pandas)를 사용하여 CSV 파일을 읽기
#  2) tf.data를 사용하여 행을 섞고 배치로 나누는 입력 파이프라인(pipeline)을 만들기
#  3) CSV의 열을 feature_column을 사용해 모델 훈련에 필요한 특성으로 매핑하기
...
# 각 행은 환자 한 명을 나타내고 각 열은 환자에 대한 속성 값
...
# 5. 입력 파이프라인 이해하기 
# 이 데이터셋은 (데이터프레임의) 열 이름을 키로 갖는 딕셔너리를 반환합니다. 
# 데이터프레임 열의 값이 매핑되어 있음
for feature_batch, label_batch in train_ds.take(1):
  print('전체 특성:', list(feature_batch.keys()))
  print('나이 특성의 배치:', feature_batch['age'])
  print('타깃의 배치:', label_batch )

# 6. 여러 종류의 특성 열 알아 보기 : 텐서플로는 여러 종류의 특성 열을 제공
...
#   6.1 수치형 열 : 특성 열의 출력은 모델의 입력이 됩니다(앞서 정의한 함수를 사용하여 데이터프레임의 각 열이 어떻게 변환되는지 알아 볼 것입니다).
#    수치형 열은 가장 간단한 종류의 열입니다. 이 열은 실수 특성을 표현하는데 사용됩니다. 
#    이 열을 사용하면 모델은 데이터프레임 열의 값을 변형시키지 않고 그대로 전달 받습니다.
age = feature_column.numeric_column("age")
demo(age)

#   6.2 버킷형 열 : 종종 모델에 수치 값을 바로 주입하기 원치 않을 때가 있습니다. 
#     대신 수치 값의 구간을 나누어 이를 기반으로 범주형으로 변환합니다. 
#     원본 데이터가 사람의 나이를 표현한다고 가정하면 나이를 수치형 열로 표현하는 대신 버킷형 열(bucketized column)을 사용하여 
#     나이를 몇 개의 버킷(bucket)으로 분할할 수 있습니다. 
#     다음에 원-핫 인코딩(one-hot encoding)된 값은 각 열이 매칭되는 나이 범위를 나타냅니다.
age_buckets = feature_column.bucketized_column(age, boundaries=[18, 25, 30, 35, 40, 45, 50, 55, 60, 65])
demo(age_buckets)

#   6.3 범주형 열 : 이 데이터셋에서 thal 열은 문자열입니다(예를 들어 'fixed', 'normal', 'reversible'). 
...
#   6.4 임베딩 열 : 가능한 문자열이 몇 개가 있는 것이 아니라 범주마다 수천 개 이상의 값이 있는 경우를 상상해 보겠습니다. 
...
#   6.5 해시 특성 열 : 가능한 값이 많은 범주형 열을 표현하는 또 다른 방법은 categorical_column_with_hash_bucket을 사용하는 것입니다. 
#      이 특성 열은 입력의 해시(hash) 값을 계산한 다음 hash_bucket_size 크기의 버킷 중 하나를 선택하여 문자열을 인코딩합니다. 
#      이 열을 사용할 때는 어휘 목록을 제공할 필요가 없고 공간을 절약하기 위해 실제 범주의 개수보다
#      훨씬 작게 해시 버킷(bucket)의 크기를 정할 수 있습니다.
...
#   6.6 교차 특성 열 : 여러 특성을 연결하여 하나의 특성으로 만드는 것을 교차 특성(feature cross)이라고 합니다. 
#      모델이 특성의 조합에 대한 가중치를 학습할 수 있습니다. 이 예제에서는 age와 thal의 교차 특성을 만들어 보겠습니다. 
#      crossed_column은 모든 가능한 조합에 대한 해시 테이블을 만들지 않고 hashed_column 매개변수를 사용하여 해시 테이블의 크기를 선택합니다.
crossed_feature = feature_column.crossed_column([age_buckets, thal], hash_bucket_size=1000)
demo(feature_column.indicator_column(crossed_feature))

# 7. 사용할 열 선택하기 : sex, cp, fbs, exang
feature_columns = []
