...
tf.model.add(tf.keras.layers.Dense(units=2, input_dim=2, activation='relu'))
# 3 deep
tf.model.add(tf.keras.layers.Dense(units=2, activation='relu'))
tf.model.add(tf.keras.layers.Dense(units=2, activation='relu'))
tf.model.add(tf.keras.layers.Dense(units=2, activation='relu'))
...
images.image.save_fig("tf2.9.3.xor_nn_3deep_relu_Adam_loss_accuracy_by_epoch_size")    
plt.show()
...
print('Accuracy(3deep,relu,Aadm): ', score[1])
